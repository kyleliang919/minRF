...
class BiRF:
    def __init__(self, model, reverse_model, ln=True):
        self.model = model
    ...
    def forward(self, x, cond):
        ...
        with torch.no_grad():
            reverse_vtheta = self.reverse_model(zt, t, cond)
        batchwise_mse = ((z1 - x - vtheta) ** 2).mean(dim=list(range(1, len(x.shape)))) + self.beta * ((reverse_vtheta + vtheta)**2).mean(dim=list(range(1, len(x.shape))))
        tlist = batchwise_mse.detach().cpu().reshape(-1).tolist()
        ttloss = [(tv, tloss) for tv, tloss in zip(t, tlist)]
        return batchwise_mse.mean(), ttloss
    
    def reverse_forward(self, x, cond):
        b = x.size(0)
        if self.ln:
            nt = torch.randn((b,)).to(x.device)
            t = torch.sigmoid(nt)
        ...
        with torch.no_grad():
            vtheta = self.model(zt, t, cond)
        reverse_vtheta = self.reverse_model(zt, t, cond)
        batchwise_mse = ((x - z1 - reverse_vtheta) ** 2).mean(dim=list(range(1, len(x.shape)))) + self.beta * ((reverse_vtheta + vtheta)**2).mean(dim=list(range(1, len(x.shape))))
        tlist = batchwise_mse.detach().cpu().reshape(-1).tolist()
        ttloss = [(tv, tloss) for tv, tloss in zip(t, tlist)]
        return batchwise_mse.mean(), ttloss
...
if __name__ == "__main__":
    ...
    rf = BiRF(model, reverse_model=reverse_model)
    optimizer = optim.Adam([each for each in model.parameters()] + [each for each in reverse_model.parameters()], lr=5e-4)
